...
def main() -> int:
    ...
    writes_visible_email_drafts = not config.combined_email_filename.startswith("__skip_")
    ...
    if config.worksheet_name not in workbook.sheetnames:
        raise ValueError(
            f"Worksheet {config.worksheet_name!r} not found. Available sheets: {', '.join(workbook.sheetnames)}"
        )
    ...
    expected_email_count = len(pending_generated_rows) if not config.combined_email_filename.startswith("__skip_") else 0
